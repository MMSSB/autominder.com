...
// //     }
// // };
// A mock translations function is included because lang.js was not provided.
// This provides the necessary text for the PDF export feature.
...
class UIManager {
    ...
    init() {
        this.initializeTheme();
        // The user requested not to translate the UI, so language methods are commented out.
        // this.initializeLanguage();
        this.bindEvents();
        this.updateStatistics();
        this.renderMaintenanceLogs();
    ...
    initializeLanguage() {
        const savedLang = 'en'; // Defaulting to English as per request
        this.applyLanguage(savedLang);
    }

    applyLanguage(lang) {
        this.currentLang = lang;
        // UI translation is disabled as per user request.
        // const translations = getTranslations(lang);
        // this.updateUIText(translations);
        document.documentElement.dir = lang === 'ar' ? 'rtl' : 'ltr';
    }
    ...
    updateUIText(translations) {
        // UI text update logic is disabled.
    }
    ...
    showSettingsModal() {
        document.getElementById('settings-car-name').value = this.storage.getCarName();
        this.showModal('settings-modal');
    }
    ...
    updateCarName() {
        ...
        if (newName) {
            this.storage.setCarName(newName);
            document.getElementById('nav-car-name').textContent = newName;
            this.showNotification('Car name updated!', 'success');
        }
    }
    ...
    createMaintenanceEntryHTML(log) {
        ...
        return `
            <div class="maintenance-entry" data-entry-id="${log.id}">
                <div class="entry-icon ${log.type}"><i class="${icon}"></i></div>
                <div class="entry-content">
                    <div class="entry-header">
                        <div class="entry-title">${typeName}</div>
                        <div class="entry-date">${formattedDate}</div>
                    </div>
                    <p class="entry-description">${log.description}</p>
                    ${log.notes ? `<p class="entry-notes" style="color: var(--text-muted); font-size: 0.8em;">${log.notes}</p>` : ''}
                    <div class="entry-meta">
                        ${mileage ? `<span><i class="fas fa-tachometer-alt"></i> ${mileage}</span>` : ''}
                        ${cost ? `<span><i class="fas fa-dollar-sign"></i> ${cost}</span>` : ''}
                    </div>
                    <div class="entry-actions">
                        <button class="btn btn-outline btn-sm" onclick="uiManager.editEntry('${log.id}')">
                            <i class="fas fa-edit"></i> Edit
...
// //             </div>
    editEntry(id) {
        const entry = this.storage.getMaintenanceLogs().find(log => log.id === id);
        if (entry) {
            document.getElementById('entry-type').value = entry.type;
            document.getElementById('entry-date').value = entry.date;
            document.getElementById('entry-description').value = entry.description;
    ...
    deleteEntry(id) {
        this.showConfirmation('Delete Entry', 'Are you sure you want to delete this entry?', () => {
            this.storage.deleteMaintenanceEntry(id);
            this.updateStatistics();
            this.renderMaintenanceLogs();
    ...
    handleSearch(term) {
        ...
        this.renderMaintenanceLogs(this.storage.searchMaintenanceLogs(term, type));
    }
    ...
    exportData() {
        const data = this.storage.exportData();
        const carName = this.storage.getCarName().replace(/[^a-z0-9]/gi, '_').toLowerCase();
        const filename = `${carName}_maintenance_${new Date().toISOString().split('T')[0]}.car`;
        this.downloadFile(data, filename, 'application/json');
        this.showNotification('Data exported successfully!', 'success');
    }
    ...
    importData() {
        ...
        input.onchange = e => {
            ...
            reader.onload = (event) => {
                ...
                if (result.success) {
                    this.showNotification(result.message, 'success');
                    window.location.reload(); // Reload to apply all imported settings
                } else {
                    this.showNotification(result.message, 'error');
                }
            };
    ...
    async exportToPDF() {
        ...
        try {
            ...
            const translations = getTranslations(isRTL ? 'ar' : 'en');
            const carName = this.storage.getCarName();
            const stats = this.storage.getStatistics();
            ...
            // --- Header ---
            doc.setFontSize(20);
            const title = `${carName} - ${translations.maintenanceReport}`;
            doc.text(title, pageWidth / 2, y, { align: 'center' });
            y += 10;
            
            doc.setFontSize(10);
            const date = `${translations.generated}: ${new Date().toLocaleDateString()}`;
            doc.text(date, pageWidth / 2, y, { align: 'center' });
            y += 15;

            // --- Stats ---
            doc.setFontSize(12);
            doc.text(`${translations.totalEntries}: ${stats.totalEntries}`, margin, y);
            doc.text(`${translations.totalCost}: $${stats.totalCost}`, pageWidth / 2, y, { align: 'center' });
            doc.text(`${translations.lastService}: ${stats.lastService}`, pageWidth - margin, y, { align: 'right' });
            y += 10;
            ...
            const body = logs.map(log => [
                this.storage.formatDate(log.date),
                this.storage.getMaintenanceTypeDisplayName(log.type),
...
// Make modal functions globally accessible from HTML onclick attributes
window.closeModal = (id) => uiManager.closeModal(id);
window.showAddEntryModal = () => uiManager.showAddEntryModal();